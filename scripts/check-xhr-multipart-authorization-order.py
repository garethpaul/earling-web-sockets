...
start_marker = "handle_call({'xhr-multipart', data, Req}"
end_marker = "%% Event management"

if source.count(start_marker) != 1 or source.count(end_marker) != 1:
    raise SystemExit("XHR multipart data handler boundaries must remain unique.")
...
for contract in (authorize, unauthorized, authorized, parse, decode, response):
    if handler.count(contract) != 1:
        raise SystemExit(
            f"XHR multipart authorization contract must contain exactly one {contract!r}."
        )
...
if not authorize_at < unauthorized_at < response_at < authorized_at < decode_at < parse_at:
    raise SystemExit(
        "XHR multipart POST data must be parsed and decoded only after Origin authorization."
    )

if parse in handler[unauthorized_at:authorized_at] or decode in handler[unauthorized_at:authorized_at]:
    raise SystemExit("Unauthorized XHR multipart handling must not parse or decode the body.")
