...
contracts = ["misultin_ws:get(headers, Ws)", "gen_server:call(Server, {websocket, authorize, Headers})", "false ->", "{ok, Origin} ->", "{session, generate, {websocket, Ws}, socketio_transport_websocket, Origin}"]
...
for contract in ("socketio_listener:authorize_origin_headers(", "socketio_listener:origins(listener(State))"):
